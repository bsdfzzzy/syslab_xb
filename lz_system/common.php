...
else
{
	...
	$map = "update lz_webdata set value=value+1 where `name`='webview'";
	$db->query($map);
	//当天访问
	$map = "delete from lz_curip where type=1 and (ip='".getIP()."' or time<".strtotime("today").")";
	$db->query($map);
	$map = "insert into lz_curip values('".getIP()."',".time().",1)";
	$db->query($map);
}
$map = "delete from lz_curip where type=0 and (ip='".getIP()."' or time<".time()."-$last_time)";
$db->query($map);
$map = "insert into lz_curip values('".getIP()."',".time().",0)";
$db->query($map);
?>