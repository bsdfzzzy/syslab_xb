...
function dict_html_pager($url,$total_page,$page = 1)
{
	$page_span = 4;
	$pager = '';
	$url = preg_replace('/\.html$/','',$url);
	if ($page>1)
		$pager .= ' <a href="'.$url.'.html" class="black_11">'.LANG_PAGER_FIRST.'</a> ';
	else
		$pager .= ' <a href="javascript:;" class="grey">'.LANG_PAGER_FIRST.'</a> ';
	if ($page>1)
		$pager.= ' <a href="'.$url.($page-1).'.html" class="black_11">'.LANG_PAGER_PREV.'</a> ';
	...
	for($i=$page-$page_span+1;$i<$page+$page_span;$i++)
	{
		...
		if ($page == $i)
			...
			$pager.= ' <a href="'.$url.$i.'.html" class="black_11">'.$i.'</a> ';
	}
...
function lz_dict_page($obj,$cond = array(),$per_page,$url='',&$info=array())
{
	global $view_data;
	$info = array();
	$cond['select'] = 'count(*)';
	...
	$cond['total'] = $per_page;
	//$pager = LANG_PAGER_TOTAL.$total.LANG_PAGER_ITEMS.$total_page.LANG_PAGER_PAGE.'<br />';
	$pager.= dict_html_pager($url,$total_page,$page);
	$view_data['pager'] = $pager;
	return $obj->get_list( $cond );
}
...
function englishsubstr($s,$begins,$end)
{
